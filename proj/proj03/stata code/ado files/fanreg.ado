...
syntax varlist [if] [,reps(integer 50) cluster(varlist) bw(real 5) np(integer 50) maxminon max(real 0) min(real 0) title(passthru) xtitle(passthru) ytitle(passthru) saving(passthru) ygen(string) xgen(string) graph(string) ylabel(passthru) scheme(passthru) yline(passthru) xline(passthru)] 
...
if "`graph'" != "off" {
	...
	if "`maxminon'" == "maxminon"  {
		twoway (line ESTFCT xgrid, lcolor(black) sort) (line ESTFCT_u xgrid, lcolor(black) sort clpat(shortdash) cmissing(n)) (line ESTFCT_l xgrid, lcolor(black) sort clpat(shortdash) cmissing(n)),  legend(off) `title' `xtitle' `ytitle' `saving' `scheme' `ylabel' `yline' `xline' yscale(range(`min' `max') xscale(range(`xmin' `xmax'))
	} 
	else {
		twoway (line ESTFCT xgrid, lcolor(black) sort) (line ESTFCT_u xgrid, lcolor(black) sort clpat(shortdash) cmissing(n)) (line ESTFCT_l xgrid, lcolor(black) sort clpat(shortdash) cmissing(n)),  legend(off) `title' `xtitle' `ytitle' `saving' `scheme' `ylabel' `yline' `xline'
	}
}
